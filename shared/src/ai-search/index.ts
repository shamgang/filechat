// COPIED FROM:
// https://github.com/langchain-ai/langchainjs/blob/2987a93902ae286481aa130344a1f977db47ba25/libs/langchain-community/src/vectorstores/azure_aisearch.ts
// The original module was not extensible enough so I copied it
...
export type AdditionalMetadataFields<AdditionalMetadata> = { [Property in keyof AdditionalMetadata]: SearchField };

/**
 * Configuration options for the `AzureAISearchStore` constructor.
 */
export interface AzureAISearchConfig<AdditionalMetadata extends Record<string, any> | undefined = undefined> {
  readonly client?: SearchClient<AzureAISearchDocument<AdditionalMetadata>>;
  readonly indexName?: string;
  readonly endpoint?: string;
  readonly key?: string;
  readonly credentials?: KeyCredential | TokenCredential;
  readonly search?: AzureAISearchQueryOptions;
  readonly additionalMetadataFields?: AdditionalMetadataFields<AdditionalMetadata>;
}
 ...
 */
export type AzureAISearchDocument<AdditionalMetadata extends Record<string, any> | undefined = undefined> = {
  id: string;
  content: string;
  content_vector: number[];
  metadata: AdditionalMetadata extends undefined ? AzureAISearchDocumentMetadata : AzureAISearchDocumentMetadata & AdditionalMetadata;
};
 ...
 */
export class AzureAISearchVectorStore<AdditionalMetadata extends Record<string, any> | undefined = undefined> extends VectorStore {
  ...
  public readonly client: SearchClient<AzureAISearchDocument<AdditionalMetadata>>;

  protected readonly indexName: string;

  private readonly options: AzureAISearchQueryOptions;

  private readonly config: AzureAISearchConfig<AdditionalMetadata>;

  constructor(embeddings: EmbeddingsInterface, config: AzureAISearchConfig<AdditionalMetadata>) {
    ...
    if (!config.client) {
      ...
      this.initPromise = this.ensureIndexExists(indexClient, config.additionalMetadataFields).catch((error) => {
        console.error(
  ...
  private async deleteMany(
    ...
    const docs: AzureAISearchDocument<AdditionalMetadata>[] = [];
    ...
    const bufferedClient =
      new SearchIndexingBufferedSender<AzureAISearchDocument<AdditionalMetadata>>(
        this.client,
        (entity) => entity.id
      );
  ...
  async addVectors(
    ...
    const entities: AzureAISearchDocument<AdditionalMetadata>[] = documents.map((doc, idx) => {
      const metadata: Record<string, unknown> = {
        attributes: []
      };
      if (this.config.additionalMetadataFields) {
        for (const { name } of Object.values(this.config.additionalMetadataFields) as SearchField[]) {
      ...
      let searchDoc = {
        ...
        metadata: metadata
      } as AzureAISearchDocument<AdditionalMetadata>;
      return searchDoc;
    });
  ...
  protected async ensureIndexExists(
    ...
    additionalMetadataFields?: AdditionalMetadataFields<AdditionalMetadata>
  ): Promise<void> {
    try {
    ...
    return {
      ...
      fields: [
        ...
        },
        {
          ...
          fields: [
            ...(additionalMetadataFields ? Object.values(additionalMetadataFields) as SearchField[] : []),
            {
